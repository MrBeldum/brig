...
| `--json` | machine-readable output, for the read verbs: `ls`, `info`, `agent ls`, `secret ls` and `doctor`. Also accepted after the verb (`brig ls --json`); every other verb refuses it. With `run`, brig runs the agent as a child and, after it exits, prints one JSON line with its exit status, so a script can tell "brig refused" from "the agent failed"; when the run booted the sandbox the line also carries `bootMillis`, the measured time from the runtime being asked to start to the guest agent first answering. Within one `apiVersion` the JSON only gains fields, never renames or drops one, and no field carries a credential value |
